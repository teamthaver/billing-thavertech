...
const AddClientPopup = ({ id, mode }: { id?: number; mode: string }) => {
  ...
  useEffect(() => {
    ...
    const allCities = City.getCitiesOfState(country.value, state.value).map(
      (c) => ({
        label: c.name,
        value: c.name,
      }),
  ...
  useEffect(() => {
    ...
    const foundCountry = countries.find((c) => c.label === data.country);
  ...
  useEffect(() => {
    ...
    const foundState = states.find((s) => s.label === data.state);
  ...
  useEffect(() => {
    ...
    const foundCity = cities.find((c) => c.label === data.city);
